...
//   export function handleSummary(data) { return generateReport(data, '脚本名'); }
...
function generateMarkdown(data, scriptName) {
...
export function generateReport(data, scriptName) {
  const now = new Date();
  const timestamp = formatTimestamp(now);
  const markdown = generateMarkdown(data, scriptName);
  const json = JSON.stringify(data, null, 2);

  return {
    // 最新报告，每次覆盖，直接看这个
    'report/report-latest.md': markdown,
    'report/report-latest.json': json,
    // 带脚本名和时间戳的归档副本
    [`report/report-${scriptName}-${timestamp}.md`]: markdown,
    [`report/report-${scriptName}-${timestamp}.json`]: json,
  };
}
