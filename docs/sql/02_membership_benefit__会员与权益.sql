...
CREATE TABLE `u_plan` (
  ...
  `display_name_en` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '套餐英文显示名，如 Plus/Max/Ultra',
  ...
  `first_month_price` decimal(10,2) DEFAULT NULL COMMENT '首月价格（仅月付周期首次购买生效，空表示不启用）',
...
CREATE TABLE `u_benefit` (
  ...
  `period_type` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'month' COMMENT '额度周期：month-自然月/day-自然日/lifetime-永久',
...
CREATE TABLE `u_benefit_usage` (
  ...
  `period` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '周期标识：月度格式 yyyy-MM、按日格式 yyyy-MM-dd，lifetime 类型权益为 lifetime',
