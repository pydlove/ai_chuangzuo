...
CREATE TABLE `u_order` (
  ...
  `amount` decimal(19,4) NOT NULL COMMENT '订单金额（现金实付，邀请分佣按此金额计算）',
  ...
  `total_amount` decimal(19,4) NOT NULL DEFAULT '0.0000' COMMENT '订单名义总金额（现金实付+创作币抵扣+优惠券抵扣），仅作记录，不参与分佣',
