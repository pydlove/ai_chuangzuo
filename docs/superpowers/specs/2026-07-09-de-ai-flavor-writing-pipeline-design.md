...
完整 12 阶段流程中，**只有 9 个阶段会调用 AI**（第 1、5、10、12 阶段是程序化，不调 AI）。每个 AI 阶段调用 1 次 LLM，共 9 次。

| 版本 | AI 调用次数 | 适用场景 |
|------|------------|----------|
| 完整版（12 阶段） | 9 次 | 深度长文、高质量要求 |
| 简化版（9 阶段） | 5 次 | 日常短文（见第 7 节） |
| 极简版（4 阶段） | 3 次 | 快生成、低质量要求 |
...
- **第 10 阶段是 deterministic 字数统计**，无需 LLM 调用
- **第 12 阶段是 deterministic 模板渲染**，规则驱动，无需 LLM 调用
- 第 7 阶段外部审视只需单角色调用一次
...
| 第 11 阶段 | 字数调整后文章是否仍然完整？核心论点是否保留？ |
| 第 12 阶段 | 模板渲染后文章内容是否被改动？各平台样式是否正确？ |
...
| **V3** | + 完整 12 阶段，含第 7 阶段外部审视 + 第 12 阶段模板渲染 | 全面质量保障 + 多平台导出 |
