...
完整 11 阶段流程中，**只有 9 个阶段会调用 AI**（第 1、5、10 阶段是程序化，不调 AI）。每个 AI 阶段调用 1 次 LLM，共 9 次。

| 版本 | AI 调用次数 | 适用场景 |
|------|------------|----------|
| 完整版（11 阶段） | 9 次 | 深度长文、高质量要求 |
| 简化版（8 阶段） | 5 次 | 日常短文（见第 7 节） |
...
| **V3** | + 完整 11 阶段，含第 7 阶段外部审视 | 全面质量保障 |
