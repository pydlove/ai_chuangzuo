...
- **分类**：在 `StylesIndex.vue` 增加第三个 tab「学习的风格」，位于「我的风格」之后、「系统预设风格」之前。创作页弹框 tab 顺序一致。
- **主色调**：「学习的风格」相关按钮、横幅、输入框 focus、上传区 hover 等均使用品牌主色 `#ff2442`（小红书红），与全局主色调一致。
- **命名**：用户手填，必填，留空时保存按钮禁用，placeholder 提示「例如：我的小红书风」。
- **适用范围**：单个 input 手填，≤ 50 字，非空。我的风格与学习的风格卡片上均以「适用：xxx」单行展示。
- **字数/大小限制**：粘贴正文最少 200 字、最多 3000 字；上传文件最大 1MB；仅支持 .txt / .md / .docx。
- **我的风格编辑器**：同步增加「适用范围」必填字段，与学习的风格保持一致。
...
- 创作页风格弹框 tab 顺序与 `StylesIndex.vue` 一致：我的风格 / 学习的风格 / 系统预设风格
- 学习的风格卡片显示来源类型 + 日期 + 适用范围 + 「使用」「查看完整」「删除」按钮
- 我的风格卡片显示适用范围（与 StylesIndex.vue 一致）
- 弹框内新建 / 编辑我的风格表单增加「适用范围」必填字段
...
1. **粘贴学习**：粘贴 > 200 字文本 → 学习 → 命名 / 适用范围 → 保存 → 卡片出现
...
12. **我的风格适用范围**：新建 / 编辑我的风格时填写适用范围，卡片显示「适用：xxx」
