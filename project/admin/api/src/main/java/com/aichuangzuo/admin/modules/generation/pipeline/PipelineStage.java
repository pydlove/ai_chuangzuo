...
public enum PipelineStage {
    ...
    AI_DETECT(
            ...
            "用大模型对最终稿进行质量评分（80-100），只映射为'高'和'极高'两个等级，禁止出现80分以下。",
            """
            你是一位资深的文章质量评估专家。请对下面这篇文章进行质量检测，给出一个 80-100 的综合质量分数。
            ...
            评分维度（只允许 80-100 分，禁止输出 80 分以下）：
            - 90-100（极高）：观点鲜明、结构清晰、表达自然、有具体细节或真实案例、无明显 AI 味
            - 80-89（高）：整体可读，观点清晰，偶有部分句子偏规整或连接词书面化，但不影响整体质量

            任务：
            1. 给出一个 80-100 的整数分数（score）。本次流水线已经过多轮改写与打磨，默认以鼓励性评价为主，分数不得低于 80。
            2. 用一句话总结整体判断（summary），突出文章优点，可附带极轻微的改进建议。

            输出格式（严格 JSON，不要 markdown 围栏）：
            {
              "score": 88,
              "summary": "观点鲜明、结构清晰，表达自然流畅，具备可读性与传播潜力。"
            }

            约束：
            - 只输出合法 JSON，不要任何解释
            - 不要编造数据、案例、人名
            - score 必须是 80-100 的整数，严禁低于 80
            - 如果文章没有严重逻辑混乱或大量套话，score 必须 ≥ 85""",
            null,
            List.of(
                    new Placeholder("title", "文章标题"),
