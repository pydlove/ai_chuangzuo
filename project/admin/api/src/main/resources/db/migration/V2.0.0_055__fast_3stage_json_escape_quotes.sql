...
-- 极速 3 阶段模板 stage 4：强化 JSON 输出格式约束，要求 content 中的双引号必须转义。
...
JSON 转义硬性要求（必须遵守，否则系统无法解析）：
- content 中如果出现双引号 "，必须转义成 \\\"。
- 例如原文：我说"没问题"。JSON 中必须写成："content": "我说\\\"没问题\\\"。"
- 如果内容里有反斜杠 \\，也要转义成 \\\\\\\\。
- 推荐做法：content 里尽量少用双引号，可用单引号或书名号代替直接引语。
- responsibility 中也不要出现未转义的双引号。
- description 中同样不要出现未转义的双引号。
...
- 只输出合法 JSON，不要任何解释、markdown 围栏或代码块标记。',
