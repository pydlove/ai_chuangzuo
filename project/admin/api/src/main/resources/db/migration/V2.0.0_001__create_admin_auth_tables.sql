...
-- 初始化超级管理员角色（幂等）
INSERT IGNORE INTO a_role (role_code, role_name, description, status) VALUES
('SUPER_ADMIN', '超级管理员', '拥有全部权限，不可删除', 1);

-- 初始化内置管理员账号 admin / Root1qaz!QAZ（幂等）
INSERT IGNORE INTO a_admin_user (username, password_hash, real_name, status, created_by, updated_by) VALUES
('admin', '$2b$12$ZWAeSaXxuQykXeM0/vTKaOlJJMvrhOWWszfDo5IEqVB48/1/qA/l2', '超级管理员', 1, 0, 0);

-- 关联超级管理员角色（幂等）
INSERT IGNORE INTO a_admin_user_role_rel (admin_user_id, role_id)
SELECT u.id, r.id
FROM a_admin_user u, a_role r
