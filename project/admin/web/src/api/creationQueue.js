...
export const listGenerationTasks = (params) =>
  request.get('/generation/tasks', { params }).then((res) => res.data)
...
export const getGenerationCallLogsGrouped = (taskId) =>
  request.get(`/generation/call-logs/by-task/${taskId}/grouped`).then((res) => res.data)

/** 在线预览已完成任务的最终文章。 */
export const previewGenerationTaskArticle = (id) =>
  request.get(`/generation/tasks/${id}/article`).then((res) => res.data)
...
export const downloadGenerationTaskArticle = (id) =>
  request.get(`/generation/tasks/${id}/article/download`, {
    responseType: 'blob'
  })
