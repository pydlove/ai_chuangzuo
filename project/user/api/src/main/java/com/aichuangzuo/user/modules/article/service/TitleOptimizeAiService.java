...
/**
 * AI 标题优化的模型调用器：读 a_model_config 选 endpoint，发送 system + user messages。
 *
 * <p>管理端 GenerationAiService 的精简版，只服务标题优化这一个同步场景，
 * 不引入 generation 配置（温度/超时直接固定）。
 */
@Slf4j
@Service
public class TitleOptimizeAiService {
     ...
     *
     * @throws BusinessException TITLE_OPTIMIZE_FAILED 配置缺失/传输失败/响应解析失败
     */
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt < MAX_ATTEMPTS) {
                    log.warn("AI 标题优化第 {}/{} 次调用失败，切换模型配置重试", attempt, MAX_ATTEMPTS);
                }
            }
        }
        throw lastError;
    ...
    private String callOnce(String systemMessage, String userMessage, int attempt) {
        ...
        if (cfg == null) {
            log.warn("AI 标题优化失败：无 active 模型配置");
            throw new BusinessException(ArticleErrorCode.TITLE_OPTIMIZE_FAILED);
        }
        log.info("AI 标题优化调用模型 第 {}/{} 次 configId={} provider={} model={}",
        ...
        try {
            ...
            log.warn("AI 标题优化 api key 解密失败 modelConfigId={}", cfg.getId(), e);
            throw new BusinessException(ArticleErrorCode.TITLE_OPTIMIZE_FAILED);
        }
        ...
        try {
            ...
            log.warn("AI 标题优化调用失败 configId={} provider={} model={} msg={}",
    ...
    private String extractContent(String responseBody, String providerType) {
        try {
            ...
            if (choices.isArray() && !choices.isEmpty()) {
                ...
                log.warn("AI 标题优化返回 content 为空 provider={} finish_reason={} hasReasoning={} (length 说明 max_tokens 被 reasoning 耗尽)",
                        providerType, first.path("finish_reason").asText(""), hasReasoning);
            }
        } catch (Exception e) {
            log.warn("AI 标题优化响应解析失败", e);
        }
        throw new BusinessException(ArticleErrorCode.TITLE_OPTIMIZE_FAILED);
    }
}
